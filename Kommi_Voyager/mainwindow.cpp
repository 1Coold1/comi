...
{
    ui->setupUi(this);
    setWindowIcon(QIcon(":/new/prefix1/Icon/road.png"));
    ...
    G->button(0)->setText("Добавить точку");
    G->button(1)->setText("←");
    G->button(2)->setText("↔");
    G->button(3)->setText("Переместить точку");
    G->button(4)->setText("Удалить точку");
    btn_calc->setText("Рассчитать маршрут");
    G->button(0)->setStyleSheet("QPushButton { background-color: rgb(147,77,191); color: rgb(222,201,201); font-size: 16px; }\n");
    G->button(1)->setStyleSheet("QPushButton { background-color: rgb(147,77,191); color: rgb(222,201,201); font-size: 16px;  }\n");
    G->button(2)->setStyleSheet("QPushButton { background-color: rgb(147,77,191); color: rgb(222,201,201); font-size: 16px;  }\n");
    G->button(3)->setStyleSheet("QPushButton { background-color: rgb(147,77,191); color: rgb(222,201,201); font-size: 16px;  }\n");
    G->button(4)->setStyleSheet("QPushButton { background-color: rgb(147,77,191); color: rgb(222,201,201); font-size: 16px;  }\n");
    btn_calc->setStyleSheet("QPushButton { background-color: rgb(147,77,191); color: rgb(222,201,201); font-size: 16px;  }\n");
    LE_ent1->setStyleSheet("border-color: rgb(10, 10, 10); font-size: 16px;  color: rgb(147,77,191); border-style: solid; border-width: 1px; border-radius: 3px");
    LE_ent2->setStyleSheet("border-color: rgb(10, 10, 10); font-size: 16px;  color: rgb(147,77,191); border-style: solid; border-width: 1px; border-radius: 3px");
    L_res->setStyleSheet("color: rgb(222,201,201); font-size: 16px; ");
    G->button(1)->setFixedSize(150,50);
    G->button(2)->setFixedSize(150,50);
    btn_calc->setFixedSize(150,50);
    LE_ent1->setFixedSize(150,50);
    LE_ent2->setFixedSize(150,50);
    grid->addWidget(openGLW,3,0,-1,-1);
    grid->addWidget(LE_ent1,1,0);
    grid->addWidget(LE_ent2,2,0);
    grid->addWidget(G->button(1),1,1, 1, 1);
    grid->addWidget(G->button(2),2,1);
    grid->addWidget(G->button(3),0,2);
    grid->addWidget(G->button(4),0,1);
    grid->addWidget(G->button(0),0,0);
    grid->addWidget(btn_calc,0,3);
    grid->addWidget(L_res, 0, 4);
...
void MainWindow::SwitchState()
{
    ...
    for (int i=0;i<5;i++)
    {
        G->button(i)->setStyleSheet("QPushButton { background-color: rgb(147,77,191); color: rgb(222,201,201); font-size: 16px; }\n");
    }
    if (buttonID!=4) G->button(buttonID)->setStyleSheet("QPushButton { background-color: rgb(147,77,191); font-size: 16px; color: rgb(222,201,201); border-color: rgb(0, 255, 0);border-style: solid; border-width: 1px; border-radius: 3px}\n");
